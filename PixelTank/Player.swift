...
final class Player : SKNode, Collidable
{
    ...
    ****************************************************************/
    func setup() ->Bool
    {
        ...
        self.physicsBody!.collisionBitMask = 0;//GameScene.CollisionInfo.AREA_MASK;
        
        // set the initial direction
        currentDirection = GameScene.Direction.DOWN;
        currentVector = CGVectorMake(0.0, -tankSpeed);
        
        // make sure the player is alive to start
        isAlive = true;
        
        // create three bullet objects
    ...
    func fireCannon()
    {
        bullets[curBulletIndex].removeFromParent();
        
        // fire the next bullet
        bullets[curBulletIndex].fireBullet(self.position, angle:self.zRotation-1.55);
        
        // readd the bullet to the scene
        self.parent!.addChild(bullets[curBulletIndex]);
        
        // move to the next bullet
        curBulletIndex++;
        if (curBulletIndex >= bullets.count)
        {
            curBulletIndex = 0;
        }
    }
    ...
    ****************************************************************/
    func turnTank(direction:GameScene.Direction)
    {
        ...
        if (direction != currentDirection)
        {
            ...
            // run the rotation
            rotateAction = SKAction.rotateToAngle(CGFloat(Player.degreesToRadians(rotateDirection)), duration: 0.1);
